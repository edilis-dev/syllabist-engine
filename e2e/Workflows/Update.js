...
const manifest = {
  actual: await Deno.readTextFile(resolve(tmp, Update.defaults.manifest)),
  expected: await Deno.readTextFile(resolve(Deno.cwd(), "Expected", Update.defaults.manifest)),
};

const parsed = JSON.parse(manifest.actual);

const artifact = {
  actual: await Deno.readTextFile(resolve(tmp, `${parsed.latest.digest}.txt`)),
  expected: await Deno.readTextFile(resolve(Deno.cwd(), "Expected", "Source.txt")),
};

assertEquals(Standardise({ data: manifest.actual }), Standardise({ data: manifest.expected }));
